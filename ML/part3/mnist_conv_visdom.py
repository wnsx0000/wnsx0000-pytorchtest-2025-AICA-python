...
for epoch in range(1, epochs + 1) :
    for batch_index, batch_data in enumerate(dataloader_train) :
        ...
        vis.line(X=torch.Tensor([visdom_index]), Y=torch.Tensor([cost.item()]), win=graph_id, update="append")
        visdom_index = visdom_index + 1

    print('Epoch {:4d}/{}, Cost: {:.6f}, visdom_index: {}'.format(epoch, epochs, cost.item(), visdom_index))
